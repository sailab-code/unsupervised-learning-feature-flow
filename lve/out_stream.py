...
class OutputType(Enum):
    ...
    def readable_type(output_type):
        if output_type == OutputType.BINARY:
            ...
            output_type = "PRIVATE"
        elif output_type == OutputType.MOTION:
            output_type = "MOTION"
        return output_type
    ...
    def from_string(string_output_type):
        ...
        elif string_output_type == "MOTION":
            output_type = OutputType.MOTION
        return output_type
...
class OutputStream:
    ...
    def save_element(self, element_name, element,
        ...
        if isinstance(element, np.ndarray) and not (data_type == OutputType.BINARY or data_type == OutputType.IMAGE
                                                    or data_type == OutputType.MOTION):
            raise ValueError("Numpy array must be saved with data types: OutputType.BINARY or OutputType.IMAGE or"
                             " OutputType.MOTION")
        ...
        elif data_type == OutputType.BINARY or data_type == OutputType.MOTION:
            if self.__gzip_bin:
