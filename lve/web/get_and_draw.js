...
function get_and_draw_motion(frame, draw_style, request) {
    let caption;
    ...
    let create_hidden_buffers = !is_canvas_existing(caption);
    var ctx = get_canvas(caption, _w, _h); // ensure this is immediately returned (to gain a top row position)
    ...
    _pending_ajax_calls.push($.ajax({
        ...
        success: function (array_buffer) {
            ...
            if (create_hidden_buffers
                || ctx.received_data_1.length !== (hh * ww)
                || ctx.received_data_2.length !== (hh * ww)) {
                ctx.received_data_1 = createNewSameType(array_of_floats, hh * ww);
                ctx.received_data_2 = createNewSameType(array_of_floats, hh * ww);
            }
            ...
            if (draw_style === 'map') {
                ...
                let epsilon = 1e-5;
                let ncols = _color_wheel.length;
                let fk, k0, k1, f, col0, col1, col;
                ...
                if (!c_first) {
                    ...
                    for (i = 0; i < array_of_floats.length; i = i + 2) {
                        ctx.received_data_1[k / 4] = array_of_floats[i];  // horizontal motion
                        ctx.received_data_2[k / 4] = array_of_floats[i + 1];  // vertical motion

                        vx = array_of_floats[i] / (max_mag + epsilon);
                        vy = array_of_floats[i + 1] / (max_mag + epsilon);
                        ...
                        // determining value for R channel
                        col0 = _color_wheel[k0][0] / 255.0;
                        col1 = _color_wheel[k1][0] / 255.0;
                        col = (1 - f)*col0 + f*col1;
                        if (mag <= 1)
                            col = 1 - mag * (1 - col);
                        else
                            col = col * 0.75;
                        data[k] = Math.floor(255 * col); // R

                        // determining value for G channel
                        col0 = _color_wheel[k0][1] / 255.0;
                        col1 = _color_wheel[k1][1] / 255.0;
                        col = (1 - f)*col0 + f*col1;
                        if (mag <= 1)
                            col = 1 - mag * (1 - col);
                        else
                            col = col * 0.75;
                        data[k + 1] = Math.floor(255 * col); // G

                        // determining value for B channel
                        col0 = _color_wheel[k0][2] / 255.0;
                        col1 = _color_wheel[k1][2] / 255.0;
                        col = (1 - f)*col0 + f*col1;
                        if (mag <= 1)
                            col = 1 - mag * (1 - col);
                        else
                            col = col * 0.75;
                        data[k + 2] = Math.floor(255 * col); // B

                        // setting value for A channel
                        data[k + 3] = 255; // A
                    ...
                    for (i = 0; i < n; i = i + 1) {
                        ctx.received_data_1[k / 4] = array_of_floats[i];
                        ctx.received_data_2[k / 4] = array_of_floats[i + n];

                        vx = array_of_floats[i] / (max_mag + epsilon);
                        vy = array_of_floats[i + n] / (max_mag + epsilon);
...
function _draw_motion_lines(canvas_title) {
    var ctx = get_canvas(canvas_title);
    var w = _w;
    var h = _h;
    var imgData = ctx.createImageData(w, h);
    var data = imgData.data;
        ...
        success: function (array_buffer) {
            ...
            if (!get_only) {
                ...
                for (f = from; f <= to; f++) {
                    ...
                    if (!Object.hasOwn(ctx, 'received_data')
                        || ctx.received_data.length !== (h * w)) {
                        ctx.received_data = createNewSameType(array_of_floats, h * w);;
                    }
                    ...
                    if (!c_first) {
                        if (normalize === "none") {
                            if (data_in_01) {
                                for (i = f; i < whc; i = i + c) {
                                    ctx.received_data[k / 4] = array_of_floats[i];
