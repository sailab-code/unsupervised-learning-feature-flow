...
var _color_wheel = generate_color_wheel(); // use to show motion predictions
...
function get_canvas(caption, w, h) {
    ...
    if (ctx === undefined) {
        ...
        if (caption === "Interaction") {
            ...
            add_mouse_listeners_to_display_clicked_value(ctx);
        }
...
// add mouse listeners to generic windows, to diplay on the the canvas caption the value of the clicked coords
function add_mouse_listeners_to_display_clicked_value(ctx) {

    // handler of all mouse-related interactions with this view
    var mouse_event_handler = function (event) {
            if (event.type === "mousedown") {
                let canvas_id = event.currentTarget.id;
                ...
                let label = "";
                if (cctx.hasOwnProperty('received_data')) {
                    label += cctx.received_data[y * _w + x];
                } else {
                    if (cctx.hasOwnProperty('received_data_1')) {
                        label += "horizontal: " + cctx.received_data_1[y * _w + x];
                        label += "<br>vertical: " + cctx.received_data_2[y * _w + x];
                    }
                }
                if (label.length > 0) {
                    $('.tooltip').html(label);
                    $('.tooltip').fadeIn().css(({left: event.pageX, top: event.pageY}));
                }
    ...
    // do not add handlers multiple times
    ctx.canvas.addEventListener('mousedown', mouse_event_handler, false);
    ctx.canvas.addEventListener('mouseleave', mouse_event_handler, false);
}
...
function do_action(action, it_is_an_inner_call) {
    ...
    if (action === 'refresh') {
        ...
        for (var output_name in params.showGenericOutputs) {
            ...
            if (params.showGenericOutputs[output_name] === "MOTION") {
                get_and_draw_motion(params.frame, params.motionDrawStyle, output_name);
            }
...
function template_minimal() {
    log("Switching to MINIMAL template");
    if (!$('#checkbox_video').is(":checked")) {
        $('#checkbox_video').click();
    }
    if (!$('#checkbox_motion').is(":checked")) {
...
function selected_an_output_from_list() {
    ...
    // loading
    $('#radio_output_normalization_none').prop('checked',
        _normalize_generic_outputs[_selected_output] === "none");
    $('#radio_output_normalization_minmax').prop('checked',
        _normalize_generic_outputs[_selected_output] === "minmax");
    $('#radio_output_normalization_gmingmax').prop('checked',
        _normalize_generic_outputs[_selected_output] === "gmingmax");
