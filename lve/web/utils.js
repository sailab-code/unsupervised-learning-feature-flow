...
function createNewSameType(array, len) {
    if (array.constructor.name === 'Float32Array') {
        return new Float32Array(len);
    } else if (array.constructor.name === 'Float64Array') {
        return new Float64Array(len);
    } else if (array.constructor.name === 'Uint8Array') {
        return new Uint8Array(len);
    } else if (array.constructor.name === 'Int8Array') {
        return new Int8Array(len);
    } else if (array.constructor.name === 'Uint16Array') {
        return new Uint16Array(len);
    } else if (array.constructor.name === 'Int16Array') {
        return new Int16Array(len);
    } else if (array.constructor.name === 'Uint32Array') {
        return new Uint32Array(len);
    } else if (array.constructor.name === 'Int32Array') {
        return new Int32Array(len);
    } else if (array.constructor.name === 'BigUint64Array') {
...
function generate_color_wheel() {
    ...
    let colorwheel = Array(ncols).fill().map(() => Array(3).fill(0));
    let col = 0

    // RY
    for (let i = 0; i < RY; i = i + 1) {
        colorwheel[i][0] = 255;
        colorwheel[i][1] = Math.floor(255 * (i / RY));
    }
    col = col + RY;

    // YG
    for (let i = 0; i < YG; i = i + 1) {
        colorwheel[col+i][0] = 255 - Math.floor(255*(i/YG));
        colorwheel[col+i][1] = 255;
    }
    col = col + YG;

    // GC
    for (let i = 0; i < GC; i = i + 1) {
        colorwheel[col+i][1] = 255;
        colorwheel[col+i][2] = Math.floor(255*(i/GC));
    }
    col = col + GC;

    // CB
    for (let i = 0; i < CB; i = i + 1) {
        colorwheel[col+i][1] = 255 - Math.floor(255*(i/CB));
        colorwheel[col+i][2] = 255;
    }
    col = col + CB;

    // BM
    for (let i = 0; i < BM; i = i + 1) {
        colorwheel[col+i][2] = 255;
        colorwheel[col+i][0] = Math.floor(255*(i/BM));
    }
    col = col + BM;

    // MR
    for (let i = 0; i < MR; i = i + 1) {
        colorwheel[col+i][2] = 255 - Math.floor(255*(i/MR));
