...
class GEymol:
    ...
    def __init__(self, parameters, device=None):
        ...
        self.static_image = hasattr(parameters, "static_image")
    ...
    def next_location(self, frame_t, of_t, lock=None, frame_gray_uint8_cpu=None,
                      virtualmass_xy=None, virtualmass_vxvy=None):
        ...
        if of_t.ndim != 4 or of_t.shape[0] != 1 or of_t.shape[1] != 2:
        ...
        if self.first_call:
            ...
            if self.static_image:
                # if processing a static image - to speed up computations # print("First time gradient!")
                self.gradient_norm_t_static = GEymol.__get_gradient_norm(frame_t, self.sobel_kernel)
                self.of_norm_t_static = GEymol.__get_opticalflow_norm(of_t)
            return self.y, False

        # computing features
        if self.static_image:
            # if processing a static image - to speed up computations
            gradient_norm_t = self.gradient_norm_t_static * (1.0 - self.IOR_matrix)
            of_norm_t = self.of_norm_t_static
        else:
            # if processing a video stream # print("Every step gradient!")
            gradient_norm_t = GEymol.__get_gradient_norm(frame_t, self.sobel_kernel) * (1.0 - self.IOR_matrix)
            of_norm_t = GEymol.__get_opticalflow_norm(of_t)

        face_map_t = self.face_map * (1.0 - self.IOR_matrix) if self.parameters['alpha_fm'] > 0.0 else None
        virtualmass_norm_t = GEymol.__get_virtualmass_norm(frame_t, virtualmass_xy) if virtualmass_xy else None
        ...
        # stacking features
        feature_maps = (gradient_norm_t, of_norm_t, face_map_t, virtualmass_norm_t)
        ...
        if not self.is_online:
            if self.t % max(int(float(self.parameters['fps']) / self.saccades_per_second), 1) == 0:
                # inhibition only done every <max(int(float(self.parameters['fps']) / self.saccades_per_second), 1)> seconds
                # print("OOOOOOOOOOOOO+++")
                self.IOR_matrix = self.__inhibit_return_in(self.IOR_matrix, row_col=foa_xy_and_velxy[0:2])
                # if self.t % 50 == 0:
                #     plt.imshow(self.IOR_matrix)
                #     plt.show()
                #     plt.close()
                # exit()
        else:
        ...
        if frame_gray_uint8_cpu is not None:
            ...
            if frame_t.dtype == torch.float32:
                faces = self.face_detector.detectMultiScale(
                    lve.utils.torch_float_01_to_np_uint8(frame_t), 1.3, 5)
            else:
    ...
    @staticmethod
    def __get_virtualmass_norm(frame_t, xy):
    ...
    def __extract_patch(frame, patch_center_xy, patch_size_xy, normalize=False):
        ...
        if t_x >= h or f_x < 0 or t_y >= w or f_y < 0:
            ...
            patch[sf_x:patch_size_xy[0] - st_x, sf_y:patch_size_xy[1] - st_y] = frame[cf_x:ct_x + odd_x,
                                                                                cf_y:ct_y + odd_y]
        else:
    ...
    def __my_ode(y, t, feature_maps, parameters, gravitational_filter):
        ...
        if feature_maps[2] is not None:
            ...
            gravitational_faces = torch.zeros_like(gravitational_grad)
        ...
        if feature_maps[3] is not None:
            ...
            gravitational_virtualmass = torch.zeros_like(gravitational_grad)
